...
class ContainerSharder(ContainerReplicator):
    ...
    def _report_stats(self):
        ...
        self.logger.info(
            ...
            '%(container_pivots)s pivots found, '
    ...
    def _get_pivot_ranges(self, account, container, newest=False):
        ...
        try:
            ...
            self.logger.error("Failed to get pivot points from %s/%s",
                              account, container)
            return None
    ...
    def _misplaced_objects(self, broker, root_account, root_container, pivot):
        ...
        if state == DB_STATE_SHARDED or broker.is_deleted():
            ...
            # object table that is less then (<) the pivot this node it up to
            # is considered misplaced, anything above is being held.
    ...
    def _audit_shard_container(self, broker, pivot, root_account=None,
        ...
        if self.ranges is None:
            ...
            self.logger.warning("Failed to get a pivot tree from root "
    ...
    def _update_pivot_ranges(self, account, container, op, pivots):
        ...
        for pivot in pivots:
            ...
            headers = {
                ...
                'x-backend-pivot-objects': pivot.object_count,
    ...
    def _find_scanner_node(self, broker):
        self.logger.info('Started searching for best node to be pivot point '
                         'scanner for %s/%s',
                         broker.account, broker.container)
        obj_count = [broker.get_info()['object_count']]
        ...
        if not self._get_quorum(broker, success=on_success):
            self.logger.info('Failed to reach quorum on a pivot scanner for '
        ...
        if not self._get_quorum(broker, op='POST', headers=headers):
            self.logger.info('Failed to set node %d as the pivot scanner '
                             'for %s/%s on remote servers',
                             scanner_id, broker.account, broker.container)
            return

        self.logger.info('Best pivot scanner for %s/%s is node %d',
    ...
    def _find_pivot_points(self, broker):
        ...
        :return:
        """
        self.logger.info('Started searching for pivot points on %s/%s',
                         broker.account, broker.container)
        ...
        if found_last():
            self.logger.info("Already found all pivots")
            return

        found_pivots = []

        for i in range(self.scanner_batch_size):
            next_pivot = broker.get_next_pivot_point(self.shard_container_size,
                                                     marker)
        ...
        if not found_pivots:
            ...
            self.logger.warning("No pivots found, something went wrong. We "
        ...
        for i, pivot in enumerate(found_pivots):
            ...
            self.stats['container_pivots'] += 1

        if i + 1 < len(found_pivots):
            found_pivots = found_pivots[:i]
            progress = ((len(pivot_ranges) + len(found_pivots)) *
                        self.split_size)
        ...
        if found_last() and not last_found:
            ...
            piv_name = self.generate_pivot_name(root_container, self.node_idx,
                                                '', timestamp)
            new_pivot = PivotRange(piv_name, timestamp, lower, cont_upper, 0,
                                   0, timestamp)
            pivot_ranges.append(new_pivot)
        ...
        self.logger.info("Scan cycle completed, found %d new pivots.",
                         len(found_pivots))
        if found_last():
            ...
            self.logger.info(" Final pivot reached.")
            return True
    ...
    def _shrink_phase_1(self, broker, root_account, root_container):
        ...
        if obj_count[0] > self.shard_container_size * self.shard_shrink_point:
            self.logger.info('After quorum check there were too many ojects '
    ...
    def _shrink_phase_2(self, broker, root_account, root_container):
        ...
        if not self._get_quorum(broker, success=is_success):
            self.logger.info('Failed to reach quorum on a empty/full pivot '
    ...
    def _shard_on_pivot(self, broker, root_account, root_container):
        ...
        if not pivots_todo:
            # This means no new pivot_ranges have been added since last cycle.
            ...
            if scan_complete and last_piv_exists:
                ...
                self.logger.info('No new pivot of %s/%s found, will try '
                                 'again next cycle',
                                 broker.account, broker.container)
                return
        ...
        for i in range(self.shard_batch_size):
            ...
            self.logger.info('Sharding %s/%s on pivot %s',
            ...
            try:
                ...
                info = new_broker.get_info()
                pivot.object_count = info['object_count']
                pivot.bytes_used = info['bytes_used']
                pivot.meta_timestamp = Timestamp(time.time())
            ...
            self.logger.info('Node %d sharded %s/%s at pivot %s.',
...
class RangeLink(object):
    ...
    """
    def __init__(self, pivot_range, upper=None):
        self._pivot = pivot_range
...
class RangeAnalyser(object):
    """Analyses a list of PivotRange objects to find the newest path,
    ...
    def _scan(self):
        try:
            for r in self.path:
                newest, complete, result = self._walk(r, r.pivot.timestamp, [])
    ...
    def analyse(self, ranges):
        """Analyse the given list of PivotRanges for a list of paths,
        newest first, and gops.

        Yields the determined paths from newest to oldest, in each case
        returning a set of unused PivotRanges and whether the path is complete.

           (<path list>, <set of other PivotRanges>, <bool complete>)
        :param ranges:
        :return: Tuple of (path, other_pivot_ranged, complete).
        """
        self._reset()
