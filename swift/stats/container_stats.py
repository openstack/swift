...
class ContainerStat(Daemon):
    ...
    def __init__(self, stats_conf):
        ...
        self.mount_check = server_conf.get('mount_check', 'true').lower() in \
                              ('true', 't', '1', 'on', 'yes', 'y')
        self.logger = \
            get_logger(stats_conf, log_route='container-stats')

    def run_once(self, *args, **kwargs):
        self.logger.info(_("Gathering container stats"))
        start = time.time()
        self.find_and_process()
        self.logger.info(
            _("Gathering container stats complete (%0.2f minutes)") %
            ((time.time() - start) / 60))
    ...
    def find_and_process(self):
        ...
        with open(tmp_filename, 'wb') as statfile:
            ...
            # Container Name, Object Count, Bytes Used
            for device in os.listdir(self.devices):
                if self.mount_check and not check_mount(self.devices, device):
                    self.logger.error(
                        _("Device %s is not mounted, skipping.") % device)
                    continue
                containers = os.path.join(self.devices,
                                        device,
                ...
                if not os.path.exists(containers):
                    self.logger.debug(_("Path %s does not exist, skipping.") %
                        containers)
                    continue
                for root, dirs, files in os.walk(containers, topdown=False):
