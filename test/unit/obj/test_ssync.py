...
class TestSsyncECReconstructorSyncJob(TestBaseSsyncEC):
    ...
    def test_sync_reconstructor_partial_rebuild(self):
        ...
        for x in range(2):
            # Trampoline twice - the receiving ObjectController is a coroutine
            # and it also creates a coroutine to write chunks to disk. Note
            # that the second of these coroutines is removed in later releases
            # by commit 4c11833.
            eventlet.sleep(0)
        self.assertIn('SSYNC', self.rx_logger.get_lines_for_level('info')[-1])
